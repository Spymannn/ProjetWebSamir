...
require '../lib/php/classes/serie.class.php';
require '../lib/php/classes/serieManager.class.php';
...
$pdf->cell(3.5,1,utf8_decode('Liste séries'),0,0,'L');	
...
$pdf->cell(16.5,.7,utf8_decode('Liste complètes des series du site'),0,0,'L',1);
...
$x = 2.5;
for($i=0;$i<count($listeSerie);$i++) {
    $pdf->SetXY($x, $y);
    $pdf->cell(5,3.7,utf8_decode($listeSerie[$i]->nomserie),1,'C',1,1);
    $pdf->SetXY($x+5,$y);
    //ici si c'est vide on fait un affichage special si pas on mets ce qui a
    //dans le resultSet ($data)
    $type = new TypeManager($db);
    $typeNom = $type->getNomType($listeSerie[$i]->idserie);
    $pdf->Cell(5,3.7,utf8_decode($typeNom),1,'C',1,1);
    $pdf->SetXY($x+9.5,$y);
    $pdf->Cell(3,3.7,$listeSerie[$i]->chaineserie,1,'C',1,1);
    $pdf->SetXY($x+3,$y);
    
    $ajout=$pdf->Image('../images/petitSeries/'.$listeSerie[$i]->idserie.'.jpg',16.5,$y);
